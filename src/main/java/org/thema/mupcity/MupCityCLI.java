...
public class MupCityCLI {
	public static void createProject(
			String name, File projectDir, File buildFile, int exp, double minSize, double maxSize,   
			boolean network, File roadFile, File facilityFile, File leisureFile,
			File busFile, File trainFile, File restrictFile, double minX, double minY, double width, double height) throws IOException, SchemaException {
		long startTime = System.currentTimeMillis();
		double seuilDensBuild = 0.0;// NO PARAMETER FOR THAT
		// empty monitor for CLI
		TaskMonitor mon = new TaskMonitor.EmptyMonitor();
		// new project
		Project project = Project.createProject(name, projectDir, buildFile, minX, minY, width, height, mon);
		...
		System.out.println("DECOMP = " + (endTime - startTime)/1000 + " s");
	}
	
	public static void performScenario(String name, File projectDir, int exp, int nMax, boolean useNoBuild, boolean strict, boolean mean, long seed, int ahpIndex, File results) throws IOException, SchemaException {
		long startTime = System.currentTimeMillis();
		// new project
		Project project = Project.load(new File(new File(projectDir, name), name + ".xml"));
		...
		long endTime = System.currentTimeMillis();
		System.out.println("CREATE SCENARIO = " + (endTime - startTime)/1000 + " s");
		startTime = endTime;
		project.performScenarioAuto(scenario);
		endTime = System.currentTimeMillis();
		System.out.println("PERFORM SCENARIO = " + (endTime - startTime)/1000 + " s");
		startTime = endTime;
		// save the project
		...
		endTime = System.currentTimeMillis();
		System.out.println("EXTRACT EVAL = " + (endTime - startTime)/1000 + " s");
	}
